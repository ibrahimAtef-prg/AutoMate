...
def main(argv=None):
    ...
    class _Report:
        pass
    report = _Report()
    report.pii_findings = report_data.get("pii_findings", [])
    report.secrets = report_data.get("secrets", [])
    report.sensitive_content = report_data.get("sensitive_content", [])
    report.risk_score = report_data.get("risk_score", 0)
