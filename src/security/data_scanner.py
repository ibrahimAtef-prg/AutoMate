...
try:
    ...
    raise ImportError(
        "pandas is required by data_scanner. Install: pip install pandas"
    ) from _e
...
class RegexDetector:
    ...
    SECRET_PATTERNS = {
        ...
        "connection_string": {
            # Bug fix: `:\/\/` was using escaped slashes which in a raw string means
            # literal backslash + slash. Use `://` directly — forward slashes are
            # not special in Python regex and don't need escaping.
            # Also use `postgre(?:s|sql)?` to match both postgres:// and postgresql://
            "pattern": r'(?:mongodb|mysql|postgre(?:s|sql)?|redis|amqp|mssql)://[^\s]+',
            "severity": "critical",
...
class DataScanner:
    ...
    def scan_dataframe(self, df, max_rows: Optional[int] = None) -> ScanReport:
        ...
        for col in df.columns:
            for row_idx, val in sample[col].items():
                if pd.isna(val):
                    continue
                val_str = str(val)
                if len(val_str) < 3:
                    continue
                all_findings.extend(self.regex_detector.scan_value(val_str, col, row_idx))
                all_findings.extend(self.entropy_detector.scan_value(val_str, col, row_idx))
    ...
    def scan_file(self, path: str) -> ScanReport:
        ...
        if ext in (".csv", ".tsv"):
            ...
            try:
                ...
                _log.warning("fallback path activated: data_scanner JSON parse failed, retrying as JSONL: %s", e)
                degraded = True
            ...
            _log.warning(
                "fallback path activated: unsupported extension '%s' in data_scanner.scan_file; attempting CSV reader",
                ext,
            )
            degraded = True
...
def main(argv=None):
    ...
    if args.output:
        ...
        _log.info("data_scanner: report saved to %s", args.output)
    else:
