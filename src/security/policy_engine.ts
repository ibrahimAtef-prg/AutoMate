...
function parseSimpleYaml(text: string): Record<string, any> {
    ...
    for (const raw of text.split('\n')) {
        ...
        if (sub2Match && section) {
            ...
            if (sub2Match[2].trim()) {
                (result[section] as Record<string, unknown>)[subSection] = coerce(sub2Match[2].trim());
            } else {
                (result[section] as Record<string, unknown>)[subSection] = (result[section] as Record<string, unknown>)[subSection] ?? {};
            }
            continue;
        }
        ...
        if (leaf4Match && section && subSection) {
            ...
            if (typeof (result[section] as Record<string, unknown>)[subSection] !== 'object') {
                (result[section] as Record<string, unknown>)[subSection] = {};
            }
            ((result[section] as Record<string, unknown>)[subSection] as Record<string, unknown>)[key] = val;
        }
    }
    return result;
}
