...
// ─────────────────────────────────────────────────────────────────────────────
// PipelineContext (mirrors openrouter_client.ts — kept in sync manually)
// ─────────────────────────────────────────────────────────────────────────────
...
export function buildDatasetContext(ctx: PipelineContext): DatasetContext {
    ...
    for (const [col, stats] of Object.entries(b.columns?.categorical ?? {})) {
        ...
        const topVals = s.top_values
            ? Object.entries(s.top_values as Record<string, number>)
    ...
    // ── Threats ───────────────────────────────────────────────────────────
    const threats = (l.threat_details ?? l.top_threats ?? []).map((t: any) => ({
        name:         t.name,
        severity:     t.severity,
    ...
    // ── Top correlations ──────────────────────────────────────────────────
    const topCorr = (g.top_correlations ?? []).slice(0, 8).map((c: any) => ({
        cols:    c.cols,
        pearson: c.pearson,
        strength: c.strength,
    }));
...
export interface StructuredDatasetContext {
    rows: number | null;
    columns: string[];               // authoritative column name list
    privacy_score: number | null;
    dataset_risk_score: number | null;
    statistical_reliability_score: number | null;
