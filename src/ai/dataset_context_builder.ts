...
import { LeakageOutput, ScanReport, BaselineArtifact, GeneratorOutput } from '../webview/types/governance';

/**
 * PipelineContext — typed version replaces the duplicate all-`any` local definition.
 * Imported by openrouter_client.ts; defined here to avoid circular dependency.
 * All fields use governance.ts types — no `any`.
 */
export interface PipelineContext {
    ...
    datasetCtx?:   DatasetContext;   // forward ref — defined below in this file
}
...
export interface RiskMetrics {
    ...
    dataset_intelligence_risk: number | null;
    intelligence_risk_label: string | null;
    privacy_score: number | null;
    privacy_score_pct: string | null;
...
export interface DatasetContext {
    ...
    threats: Array<{ name: string; severity: string; confidence: number; description: string; triggered_by: string[] }>;
    top_correlations: Array<{ cols: string; pearson: number; strength: string }>;
    recent_alerts: SecurityAlert[];
    has_data: boolean;
    built_at: string;
    /** Raw sample rows (max 20) — passed through to LLM context, never fabricated. */
    sample_rows?: Record<string, unknown>[];
}

// PipelineContext is imported from openrouter_client.ts — single definition.
// The local duplicate (all `any`) is removed; import is added below.

// ─────────────────────────────────────────────────────────────────────────────
// Builder
// ─────────────────────────────────────────────────────────────────────────────

export function buildDatasetContext(ctx: PipelineContext): DatasetContext {
    // Typed access — ctx fields are now LeakageOutput, BaselineArtifact etc. not `any`
    const b  = ctx.baseline   ?? ({} as BaselineArtifact);
    const l  = ctx.leakage    ?? ({} as LeakageOutput);
    const r  = ctx.result     ?? ({} as GeneratorOutput);
    const sc = ctx.scanReport ?? ({} as ScanReport);
    ...
    const meta     = b.meta ?? {} as Partial<BaselineArtifact['meta']>;
    const profile  = ((ctx.ast?.dataset ?? ctx.ast ?? {}) as Record<string, unknown>).profile as Record<string, unknown> ?? {};

    // ── Dataset summary ───────────────────────────────────────────────────
    const summary: DatasetSummary = {
        rows:                    (b.meta?.row_count ?? (profile.row_count_estimate as number | null) ?? null),
        columns:                 (allCols.length || b.meta?.column_count) ?? 0,
        ...
        source_file:             b.meta?.dataset_source ?? null,
        generator_used:          r.generator_used ?? null,
        synthetic_rows:          r.row_count ?? null,
    };
    // Context built successfully — summary.rows available for debugging if needed
    ...
    // ── Privacy components ────────────────────────────────────────────────
    const pc: PrivacyComponents | null = l.privacy_components
        ? {
    ...
    const piiColsUnique = [...new Set(piiCols)] as string[];

    // ── Sensitive column ranking ──────────────────────────────────────────
    const sensitiveColumns: SensitiveColumn[] = (l.sensitive_column_ranking ?? [])
        .slice(0, 12)
        .map((item) => ({
            column:               item.column,
            score:                item.score ?? 0,
            pii_score:            item.pii_score ?? 0,
            reidentification_risk: item.reidentification_risk ?? 0,
    ...
    for (const [col, stats] of Object.entries(b.columns?.numeric ?? {})) {
        ...
        columnStats.push({
            ...
            mean:      s.mean   ?? undefined, std:  s.std  ?? undefined,
    ...
    for (const [col, stats] of Object.entries(b.columns?.categorical ?? {})) {
        const s = stats;
        const topVals: string[] = (s.top_values ?? [])
    ...
    for (const ot of (l.outlier_risk ?? []).slice(0, 5)) {
        anomalies.push({
            ...
            detail: `value=${ot.value} — ${ot.description ?? 'individual may be re-identifiable via outlier'}`,
        });
    }
    ...
    const govActionsUnique = govActions.filter((a, i, arr) =>
        i === arr.findIndex(b => b.column === a.column && b.action === a.action)
    );

    // ── Threats ───────────────────────────────────────────────────────────
    const threats = (l.threat_details ?? []).map((t) => ({
        name:         t.name,
        severity:     t.severity,
    ...
    // ── Top correlations ──────────────────────────────────────────────────
    const topCorr: Array<{ cols: string; pearson: number; strength: string }> =
        ((g['top_correlations'] ?? []) as Array<Record<string,unknown>>).slice(0, 8).map((c) => ({
            cols:     String(c['cols']    ?? ''),
            pearson:  Number(c['pearson'] ?? 0),
...
export const AgentTools = {
    get_dataset_summary(ctx: DatasetContext): DatasetSummary {
        return ctx.dataset_summary;
    },

    get_sensitive_columns(ctx: DatasetContext): SensitiveColumn[] {
        return ctx.sensitive_columns;
    },

    get_privacy_metrics(ctx: DatasetContext): {
        risk_metrics: RiskMetrics;
        privacy_components: PrivacyComponents | null;
        threats: DatasetContext['threats'];
    } {
        return {
    ...
    get_pii_findings(ctx: DatasetContext): {
        pii_columns: string[];
        cleaning_suggestions: CleaningSuggestion[];
        governance_actions: GovernanceAction[];
    } {
        return {
            ...
            cleaning_suggestions: ctx.cleaning_suggestions.filter(s =>
                ctx.pii_columns.includes(s.column)),
            governance_actions: ctx.governance_actions,
        };
    },

    get_recent_alerts(ctx: DatasetContext): SecurityAlert[] {
        return ctx.recent_alerts;
    },

    get_anomalies(ctx: DatasetContext): AnomalySignal[] {
        return ctx.anomalies;
    },

    get_column_stats(ctx: DatasetContext, columnName?: string): ColumnStats[] {
        if (columnName) {
    ...
    get_sql_schema(ctx: DatasetContext): { table: string; columns: Array<{ name: string; type: string; nullable: boolean; is_pii: boolean }> } {
        return {
...
export function formatContextForLLM(ctx: DatasetContext): string {
    const lines: string[] = [];
    ...
    if (s.generator_used) { lines.push(`  Generator: ${s.generator_used} | Synthetic rows: ${s.synthetic_rows}`); }
    lines.push('');
    ...
    if (ctx.sensitive_columns.length > 0) {
        ...
        ctx.sensitive_columns.slice(0, 8).forEach((sc, i) => {
            lines.push(
                `  ${i + 1}. ${sc.column}: score=${sc.score.toFixed(3)}` +
    ...
    if (ctx.threats.length > 0) {
        ...
        ctx.threats.slice(0, 5).forEach(t => {
            ...
            if (t.triggered_by.length > 0) { lines.push(`    Triggered by: ${t.triggered_by.join(', ')}`); }
        });
        lines.push('');
    }
...
// ─────────────────────────────────────────────────────────────────────────────
// Phase 1 — Structured DATASET_CONTEXT block
// This is the ONLY source of ground-truth facts that the governance-analyst
// LLM is allowed to reference.  Values come 1-to-1 from the pipeline outputs;
// nothing is invented or estimated here.
// ─────────────────────────────────────────────────────────────────────────────

/**
 * StructuredDatasetContext — the ONLY data structure passed to the LLM prompt builder.
 *
 * All 20 fields accessed by openrouter_client.ts are declared here.
 * If you add a field to the prompt builder, add it here first.
 * If a field is removed here, TypeScript compilation fails at every usage site.
 *
 * ISS-001 fix: was missing 16 fields that openrouter_client.ts accessed as sdc.X.
 * Those fields resolved to undefined at runtime, silently producing 'N/A' in prompts.
 */
export interface StructuredDatasetContext {
    // ── Identity ───────────────────────────────────────────────────────────
    rows: number | null;
    columns: string[];                      // all column names (numeric + categorical)
    column_names: string[];                 // alias — same list, explicit for openrouter
    numeric_column_names: string[];
    categorical_column_names: string[];

    // ── Risk metrics ───────────────────────────────────────────────────────
    privacy_score: number | null;
    privacy_score_pct: string | null;       // formatted "72.3%" string for display
    dataset_risk_score: number | null;
    statistical_reliability_score: number | null;
    ...
    column_drift: Record<string, number>;
    pii_columns: string[];
    sensitive_column_ranking: Array<{
        column: string;
        score: number;
        pii_score: number;
        reidentification_risk: number;
    ...
    generator_used: string | null;
    synthetic_rows: number | null;
    sample_rows: Record<string, unknown>[];  // max 20 rows

    // ── Backward compat ────────────────────────────────────────────────────
    sensitive_columns: Array<{ name: string; reid_score: number | null; is_pii: boolean }>;
    recent_security_alerts: string[];
}
 ...
 */
/**
 * Build the canonical StructuredDatasetContext.
 *
 * ISS-001 + ISS-007 fix: was returning 9 fields; openrouter_client.ts accesses 20.
 * Now all 20 fields are populated from ctx — nothing is dropped or undefined.
 *
 * Sample rows are capped at 20 to prevent LLM context bloat.
 */
export function buildStructuredDatasetContext(ctx: DatasetContext): StructuredDatasetContext {
    const s = ctx.dataset_summary;
    const r = ctx.risk_metrics;

    // ── Column lists ───────────────────────────────────────────────────────
    const numCols  = s.numeric_column_names  ?? [];
    const catCols  = s.categorical_column_names ?? [];
    const allCols  = [...numCols, ...catCols];

    // ── Privacy score formatted for display ────────────────────────────────
    const privacyScorePct: string | null = r.privacy_score != null
        ? `${(r.privacy_score * 100).toFixed(1)}%`
        : null;

    // ── Privacy components ─────────────────────────────────────────────────
    const privacyComponents = ctx.privacy_components
        ? {
            duplicates_risk:           ctx.privacy_components.duplicates_risk,
            mi_attack_risk:            ctx.privacy_components.mi_attack_risk,
            distance_similarity_risk:  ctx.privacy_components.distance_similarity_risk,
            distribution_drift_risk:   ctx.privacy_components.distribution_drift_risk,
          }
        : null;

    // ── Sensitive column ranking ───────────────────────────────────────────
    const sensitiveColRanking = ctx.sensitive_columns.map(sc => ({
        column:                sc.column,
        score:                 sc.score,
        pii_score:             sc.pii_score,
        reidentification_risk: sc.reidentification_risk,
        drift_score:           sc.drift_score,
        signals:               [],
    }));

    // ── Backward-compat sensitive_columns list ────────────────────────────
    ...
    // ── Threats (normalised from threat_details) ──────────────────────────
    const threats = ctx.threats.map(t => ({
        name:        t.name,
        severity:    t.severity,
    ...
    // ── Recent security alerts (max 10) ────────────────────────────────────
    const alertStrings = ctx.recent_alerts.slice(0, 10).map(a =>
        `[${a.severity.toUpperCase()}] ${a.type} — ${a.pattern} (file: ${a.file})`
    );

    // ── Sample rows (cap at 20 to avoid context bloat) ────────────────────
    const sampleRows: Record<string, unknown>[] = (ctx.sample_rows ?? []).slice(0, 20);

    return {
        // Identity
        rows:                          s.rows,
        columns:                       allCols,
        column_names:                  allCols,        // explicit alias for openrouter
        numeric_column_names:          numCols,
        categorical_column_names:      catCols,

        // Risk metrics
        privacy_score:                 r.privacy_score,
        privacy_score_pct:             privacyScorePct,
        dataset_risk_score:            r.dataset_risk_score,
        statistical_reliability_score: r.statistical_reliability_score,
        risk_level:                    r.risk_level,
        membership_inference_auc:      r.membership_inference_auc,
        duplicates_rate:               r.duplicates_rate,
        ...
        pii_columns:                   ctx.pii_columns,
        sensitive_column_ranking:      sensitiveColRanking,

        // Threats
        threats,

        // Synthetic data context
        generator_used:  s.generator_used,
        synthetic_rows:  s.synthetic_rows,
 ...
 */
export function formatStructuredDatasetContext(sdc: StructuredDatasetContext): string {
    const lines: string[] = [];
 ...
 */
export function getValidNumbers(sdc: StructuredDatasetContext): Set<string> {
