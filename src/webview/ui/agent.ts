...
<div id="pane-aiinsights" class="tabpane">
  ...
  <div id="ai-insights" style="margin-top:10px;padding:8px;border:1px solid var(--bd);border-radius:8px;background:var(--panel)">
    <div style="color:var(--fg3);font-size:11px">Insights will appear after pipeline runs.</div>
  </div>
...
function initAgentChat(){
  ...
  // Restore saved provider selection and its key
  try{
    var savedProvider=localStorage.getItem('automate_api_provider')||'openrouter';
    var sel=document.getElementById('agent-provider');
    if(sel) sel.value=savedProvider;
    var savedKey=localStorage.getItem('automate_api_key_'+savedProvider);
    var stLabel=document.getElementById('agent-key-status');
    var stInp=document.getElementById('agent-api-key');
    var info=(PROVIDER_INFO&&PROVIDER_INFO[savedProvider])||{label:savedProvider,placeholder:'Paste API key'};
    if(savedKey){
      vscode.postMessage({command:'setApiKey',apiKey:savedKey,provider:savedProvider});
      if(stLabel){ stLabel.style.color='var(--green)'; stLabel.textContent='✓ '+info.label+' key loaded'; }
      if(stInp) stInp.placeholder='Key active — paste a new key to update';
    } else {
      if(stLabel){ stLabel.style.color='#fb923c'; stLabel.textContent='No key for '+info.label; }
    }
...
function agentInitCtxBar(){
  ...
  // Load any previously saved key from localStorage and pre-send it
  try{
    var savedKey = localStorage.getItem('automate_api_key');
    if(savedKey){
      vscode.postMessage({command:'setApiKey', apiKey:savedKey});
      if(inp) inp.placeholder='Key active — paste a new key to update';
      if(status){ status.style.color='var(--green)'; status.textContent='✓ Key loaded'; }
    }
  }catch(e){}
