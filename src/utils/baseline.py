...
# ── Error types (CORE routing paths → fail-fast) ─────────────────────────────
import sys as _sys_bl, importlib.util as _ilu_bl, os as _os_bl
_errs_path_bl = _os_bl.path.normpath(_os_bl.path.join(_os_bl.path.dirname(_os_bl.path.abspath(__file__)), "errors.py"))
if "utils.errors" in _sys_bl.modules:
    _errs_bl = _sys_bl.modules["utils.errors"]
    _sys_bl.modules.setdefault("errors", _errs_bl)
elif "errors" in _sys_bl.modules:
    _errs_bl = _sys_bl.modules["errors"]
    _sys_bl.modules["utils.errors"] = _errs_bl
else:
    _errs_spec_bl = _ilu_bl.spec_from_file_location("utils.errors", _errs_path_bl)
    _errs_bl = _ilu_bl.module_from_spec(_errs_spec_bl)
    _sys_bl.modules["utils.errors"] = _errs_bl; _sys_bl.modules["errors"] = _errs_bl
    _errs_spec_bl.loader.exec_module(_errs_bl)
_InputValidationError_bl = _errs_bl.InputValidationError
_ErrorCode_bl            = _errs_bl.ErrorCode
...
try:
    ...
    raise ImportError(
        "pandas is required by baseline. Install: pip install pandas"
    ) from _e
...
class NumericColumnBaseline:
    ...
    # Precision mode for generator normalisation.
    # Set once during baseline analysis from observed data range only —
    # no column-name heuristics.  Valid values:
    #   "continuous"  — general real-valued columns
    #   "probability" — observed range ⊆ [0.0, 1.0]  (exact, no tolerance)
    #   "percentage"  — observed range ⊆ [0.0, 100.0] (exact, no tolerance)
    precision_mode: str = "continuous"
...
def _dataset_health(df: Any) -> Dict[str, Any]:
    return {
        "rows": len(df),
        "cols": len(df.columns),
...
def build_baseline(
    ...
    *,
    ...
    """
    if isinstance(dataset_ast, str):
        # Service-layer convenience: accept raw dataset path and parse it first.
        try:
            from .parse import parse_dataset  # type: ignore
        except Exception:  # pragma: no cover - script-mode fallback
            from parse import parse_dataset  # type: ignore
        dataset_ast = parse_dataset(dataset_ast)
    ...
    if len(df) < 10:
        raise _InputValidationError_bl(
            _ErrorCode_bl.EMPTY_DATASET,
            "Dataset too small (<10 rows) — results are not reliable"
        )

    meta.row_count = int(df.shape[0])
    meta.column_count = int(df.shape[1])
    meta.dataset_health = _dataset_health(df)
    ...
    for col in numeric_cols:
        ...
        if len(non_null):
            ...
            # Precision mode: determined once from observed value range.
            # Classification is exact — no floating-point tolerance is applied.
            #
            # Rules (evaluated in priority order):
            #   Ph-3: binary (unique_count <= 2) → "categorical"  ALWAYS.
            #       A column with only 2 distinct values is a flag/indicator,
            #       never a probability distribution, regardless of its range.
            #   low-cardinality (unique_count <= 10) → "categorical"
            #       Discrete numeric columns treated as categorical indicators.
            #   [0, 1]   → "probability"  — ONLY when unique_count > 10
            #   [0, 100] → "percentage"   — ONLY when unique_count > 10
            #   else     → "continuous"
            _is_binary   = (nb.unique_count is not None and nb.unique_count <= 2)
            _is_low_card = (nb.unique_count is not None and nb.unique_count <= 10)
            if _is_binary:
                # Ph-3: binary columns are NEVER probability distributions
                nb.precision_mode = "categorical"
            elif _is_low_card:
                # Discrete indicator — not a statistical distribution.
                nb.precision_mode = "categorical"
            elif (
                not _is_low_card
                and nb.min is not None and nb.max is not None
                and nb.min >= 0.0
                and nb.max <= 1.0
            ):
                nb.precision_mode = "probability"
            elif (
                not _is_low_card
                and nb.min is not None and nb.max is not None
                and nb.min >= 0.0
                ...
                nb.precision_mode = "percentage"
            else:
                nb.precision_mode = "continuous"
    ...
    for col in categorical_cols:
        ...
        if unique_count > 0:
            ...
            # Ph-15: warn when top_k truncation drops classes
            if len(top_values) < unique_count:
                n_dropped = unique_count - len(top_values)
                meta.warnings.append(
                    f"Label distribution truncated for column '{col}': "
                    f"top-{top_k} kept, {n_dropped} class(es) removed. "
                    "These classes will not appear in generated data."
                )
        ...
        # Fix 3: Extreme imbalance warning (>90% single class)
        if top_values:
            total_count = sum(top_values.values())
            if total_count > 0:
                max_ratio = max(top_values.values()) / total_count
                if max_ratio > 0.9:
                    meta.warnings.append(
                        f"Column '{col}' is highly imbalanced (>90% single class)"
                    )

        # Fix 4: High cardinality warning (>100 unique values)
        if unique_count > 100:
            meta.warnings.append(
                f"Column '{col}' has high cardinality (>100 unique values)"
            )
    ...
    if max_rows is not None:
        meta.warnings.append(f"Baseline built in bounded mode using random sampling (n={max_rows}, random_state=42) — unbiased representation of dataset.")
...
def _load_dataframe(kind: str, source: str, *, max_rows: Optional[int]) -> Tuple[any, List[str]]:
    ...
    if kind == "csv":
        ...
        if max_rows is not None and len(df) > max_rows:
            ...
            warnings.append(f"Baseline built using random sampling (n={max_rows}, random_state=42) — unbiased.")
        return df, warnings
    ...
    if kind == "json":
        ...
        try:
            ...
            _log.warning("fallback path activated: baseline JSON parse failed, retrying as JSONL: %s", e1)
            # Newlines / nested JSON
            try:
                df = pd.read_json(source, lines=True)
            except ValueError as e2:
                _log.warning("fallback path activated: baseline JSONL parse failed, using json_normalize: %s", e2)
                try:
                    obj = json.loads(_read_text(source))
                    df = pd.json_normalize(obj)
                except (ValueError, TypeError, json.JSONDecodeError) as e3:
                    raise ValueError(
                        "Failed to parse JSON via standard, lines, and normalize paths"
                    ) from e3
        if max_rows is not None and len(df) > max_rows:
            df = df.sample(n=max_rows, random_state=42).reset_index(drop=True)
            warnings.append(f"Baseline built using random sampling (n={max_rows}, random_state=42) — unbiased.")
    ...
    if kind == "sql":
        raise _InputValidationError_bl(_ErrorCode_bl.UNSUPPORTED_FORMAT, "baseline.py expects a tabular dataset source, not raw SQL. Execute SQL to materialize data first.")

    raise _InputValidationError_bl(_ErrorCode_bl.UNSUPPORTED_FORMAT, f"Unsupported kind for baseline loading: {kind}")
...
def build_baseline_from_parsed_ast_json(
    ...
    **kwargs: Any,
    ...
    ds = obj.get("dataset")
    if not ds:
        raise _InputValidationError_bl(_ErrorCode_bl.MISSING_FIELD, "Parsed AST JSON does not contain 'dataset'.")
...
def _main(argv: Optional[List[str]] = None) -> int:
    ...
    _sys_bl.stdout.write(baseline.to_json(indent=2) + "\n")
    return 0
...
def _infer_kind_from_path(path: str) -> str:
    ...
    if ext == ".parquet":
        return "parquet"
    raise _InputValidationError_bl(_ErrorCode_bl.UNSUPPORTED_FORMAT, f"Cannot infer kind from extension: {ext}")
