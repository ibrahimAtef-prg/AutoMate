...
class ConstraintFilter:
    ...
    def apply(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, int]:
        ...
        for col, (lo, hi) in bl.num_ranges.items():
            ...
            # Phase 5: inverted range is physically impossible — fail fast rather
            # than silently clamping every value to the wrong boundary.
            if lo_f > hi_f:
                raise ValueError(
                    f"ConstraintFilter: column '{col}' has inverted range "
                    f"[{lo_f}, {hi_f}] (lo > hi). "
                    "The baseline spec is corrupted or invalid. "
                    "Re-run baseline analysis to regenerate valid constraints."
                )
...
class RowQualityFilter:
    ...
    def __init__(self, bl: Any, max_failures: int = 1) -> None:
        ...
        for col, spec in bl.numeric.items():
            ...
            if iqr <= 0:
                # Ph-4: constant or near-constant column.
                # If std > 0, use mean ± 3*std as the fence (catches true outliers).
                # If std == 0, enforce exact match by collapsing lo/hi to the single value.
                mean = spec.get("mean")
                std  = spec.get("std")
                if mean is not None and std is not None and float(std) > 0:
                    s = float(std)
                    ...
                    # Truly constant column — any deviation is an outlier
                    self._iqr_fences[col] = (float(mean), float(mean))
                continue
            self._iqr_fences[col] = (
...
def _resample_out_of_range(
    ...
    if len(remaining) > 0:
        raise ValueError(
            f"Resampling failed after {max_attempts} attempts — cannot satisfy bounds [{lo}, {hi}]"
        )
    return arr