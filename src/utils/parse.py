...
def parse_dataset(
    ...
    **kwargs: Any,
    ...
    """
    Convenience API for service layers.
    Returns only DatasetAST payload as a plain dict.
    """
    parsed = parse(source, kind=kind, **kwargs).to_dict()
    dataset = parsed.get("dataset")
    if dataset is None:
        raise ValueError("Input did not produce a dataset AST")
    return dataset
...
def _infer_kind(source: str) -> str:
    """
    Infer the data kind from a file path.

    Ph-19: If the source looks like a file path (has an extension, or is an
    absolute/relative path) but the file does not exist, raise FileNotFoundError
    immediately. Do NOT silently fall through to SQL treatment — a missing file
    is always a caller error, not an SQL query.
    """
    # Determine if source looks like a file path (has an extension or path sep)
    _looks_like_path = (
        os.sep in source
        or os.path.splitext(source)[1] != ""
    ...
    if _looks_like_path:
        if not os.path.exists(source):
            raise FileNotFoundError(
                f"parse._infer_kind(): file not found: '{source}'. "
                "Provide a valid file path (csv/json/parquet/excel) or explicit kind='sql' "
                "for SQL text input. [Ph-19]"
            )
        if not os.path.isfile(source):
            raise FileNotFoundError(
                f"parse._infer_kind(): path exists but is not a file: '{source}'. [Ph-19]"
            )
    ...
    if os.path.exists(source) and os.path.isfile(source):
        ...
        if ext in [".sql"]:
            return "sql"
        return "csv"  # safest: treat unknown extension as delimited text
    # No file indicator → treat as SQL text (explicit opt-in path)
    return "sql"
...
def _coerce_json_records(obj: Any, records_path: Optional[List[Union[str, int]]]) -> Tuple[List[Dict[str, Any]], List[str]]:
    ...
    if records_path:
        try:
            ...
            _log.warning("fallback path activated: records_path follow failed, using heuristic path discovery: %s", e)
...
def _compute_profile(rows: List[Dict[str, Any]], schema: SchemaAST) -> ProfileAST:
    prof = ProfileAST()
    row_count = len(rows)
    prof.row_count_estimate = row_count
...
def _parse_csv(path: str, *, sample_rows: int, encoding: str, dialect: str) -> DatasetAST:
    ...
    # Fix 8: Fast line count to detect large files before sampling
    try:
        with open(path, "rb") as _f:
            _line_count = sum(1 for _ in _f)
        if _line_count > 100001:  # header + >100k data rows — matches Parquet threshold of >100000 rows
            warnings.append(
                f"Large dataset detected (>{_line_count - 1:,} rows) — analysis uses a sample; full dataset may require more resources"
            )
    except OSError as e:
        _log.warning("fallback path activated: CSV pre-scan line count unavailable: %s", e)
        warnings.append(f"CSV pre-scan unavailable; proceeding without large-file warning. Details: {e}")

    rows: List[Dict[str, Any]] = []
    schema: SchemaAST

    _ROW_HARD_LIMIT = 1_000_000

    # Prefer pandas if available for better delimiter inference, but keep stdlib fallback.
    if pd is not None:
        # Encoding fallback: try requested encoding first, then latin-1
        _encodings_to_try = [encoding] if encoding != "latin-1" else [encoding]
        if encoding != "latin-1":
            _encodings_to_try.append("latin-1")
        ...
        if _enc_error is not None:
            _log.warning(
                "fallback path activated: all encodings failed for '%s'; using stdlib csv",
                path,
            )
            warnings.append(
                f"Encoding detection failed ({_enc_error}); falling back to stdlib csv reader."
            )

        if _enc_error is None:
            try:
                # F-9 (Phase 8): on_bad_lines="error" makes pandas raise immediately
                # when any data row has a different number of fields than the header.
                df = pd.read_csv(
                    path,
                    nrows=min(sample_rows, _ROW_HARD_LIMIT),
                    encoding=_enc_used,
                    sep=None,
                    engine="python",
                    on_bad_lines="error",
                ...
                # Phase 15: Reject malformed CSVs that produce None column names.
                # This happens when a data row has more fields than the header row.
                # A None key propagates into downstream JSON as "null" — a schema-
                # breaking transformation that no consumer handles correctly.
                none_cols = [c for c in df.columns if c is None]
                if none_cols:
                    raise ValueError(
                        f"parse._parse_csv(): malformed CSV at '{path}' — "
                        f"{len(none_cols)} column name(s) are None. "
                        "This usually means a data row has more fields than the header. "
                        "Fix the source file so every row has the same number of fields."
                    )
                ...
                _log.warning("fallback path activated: pandas CSV parser failed, using stdlib csv: %s", e)
                warnings.append(f"pandas CSV parser failed; using stdlib csv. Details: {e}")
            except UnicodeDecodeError as e:
                _log.warning("fallback path activated: pandas CSV decode failed, using stdlib csv: %s", e)
                warnings.append(f"pandas CSV decode failed; using stdlib csv. Details: {e}")
            except ValueError as e:
                _log.error("[HARD FAIL] parse._parse_csv invalid CSV invariant: %s", e)
                raise RuntimeError(f"[HARD FAIL] parse._parse_csv invalid CSV invariant: {e}") from e
            except Exception as e:
                _log.error("[HARD FAIL] unexpected pandas CSV failure: %s", e)
                raise RuntimeError(f"[HARD FAIL] unexpected pandas CSV failure: {e}") from e
    ...
    with open(path, "r", encoding=encoding, errors="replace", newline="") as f:
        try:
            ...
            _log.warning("fallback path activated: invalid CSV dialect '%s', using csv.excel: %s", dialect, e)
        ...
        for i, row in enumerate(reader):
            if i >= sample_rows:
                break
            # Phase 15: DictReader assigns None as key for overflow fields
            if None in row:
                raise ValueError(
                    f"parse._parse_csv(): malformed CSV at '{path}' — "
                    "a data row has more fields than the header row (stdlib csv path). "
                    "Fix the source file so every row has the same number of fields."
                )
    ...
    def rec(x: Any, p: str) -> None:
        if isinstance(x, dict):
            ...
            try:
                ...
                raise RuntimeError(f"[HARD FAIL] list serialization failed at '{p or 'value'}': {e}") from e
        else:
            out[p or "value"] = x
...
def _parse_parquet(path: str, *, sample_rows: int) -> DatasetAST:
    ...
    try:
        ...
        if row_count > 1_000_000:
            _log.warning("large dataset detected — sampling applied")
            df = df.sample(n=100000, random_state=42)
        if row_count > 100000:
            warnings.append(
                f"Large dataset detected (>{row_count:,} rows) — analysis uses a sample; full dataset may require more resources"
            )
    ...
    if sqlparse is not None:
        try:
            ...
            if not parsed:
                ...
                _log.warning("fallback path activated: sqlparse returned no statements")
            ...
            _log.warning("fallback path activated: sqlparse failed, using regex fallback: %s", e)
            stmt.warnings.append(f"sqlparse failed; using fallback. Details: {e}")
            return _parse_sql_fallback(raw, stmt)
        except Exception as e:
            _log.error("[HARD FAIL] unexpected sqlparse failure: %s", e)
            raise RuntimeError(f"[HARD FAIL] unexpected sqlparse failure: {e}") from e

    _log.warning("fallback path activated: sqlparse dependency unavailable, using regex fallback")
...
def _extract_sql_struct_with_sqlparse(statement: Any, out: SQLStatementAST) -> None:
    ...
    if "select" in lower:
        try:
            ...
            _log.warning("fallback path activated: sqlparse SELECT-column extraction failed: %s", e)
...
def _parse_sql_fallback(raw: str, stmt: SQLStatementAST) -> SQLStatementAST:
    ...
    if re.search(r"^\s*select\b", s, flags=re.I):
        try:
            ...
            _log.warning("fallback path activated: regex SQL column extraction failed: %s", e)
