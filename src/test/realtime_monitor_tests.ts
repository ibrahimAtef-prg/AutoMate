...
const _notifications: Array<{ level: string; msg: string }> = [];
(global as any).__vscode_stub = {
    window: {
        showErrorMessage:   (m: string) => { _notifications.push({ level: 'error',   msg: m }); },
        showWarningMessage: (m: string) => { _notifications.push({ level: 'warning', msg: m }); },
        showInformationMessage: (m: string) => { _notifications.push({ level: 'info', msg: m }); },
    },
...
// ─────────────────────────────────────────────────────────────────────────────
// Lightweight test harness (no external test runner required)
// ─────────────────────────────────────────────────────────────────────────────
...
function assertContains(haystack: string, needle: string, msg?: string): void {
    assert.ok(
        ...
        msg ?? `Expected "${haystack.substring(0, 120)}" to contain "${needle}"`
    );
}

function assertGreater(actual: number, threshold: number, msg?: string): void {
    assert.ok(actual > threshold, msg ?? `Expected ${actual} > ${threshold}`);
}

function assertLength(arr: any[], minLen: number, msg?: string): void {
    assert.ok(arr.length >= minLen, msg ?? `Expected array length >= ${minLen}, got ${arr.length}`);
}

// ─────────────────────────────────────────────────────────────────────────────
// Inline re-implementation of the detection engines
// (mirrors production code exactly — no VS Code API surface needed)
// ─────────────────────────────────────────────────────────────────────────────

/* ---------- SECRET DETECTION PATTERNS (mirrors realtime_scanner.ts) -------- */
...
let _store: Alert[] = [];
const _listeners: Array<(a: Alert) => void> = [];
let _counter = 0;

function pushAlert(a: Alert): void {
    _store.unshift(a);
    if (_store.length > 200) { _store.length = 200; }
    _listeners.forEach(fn => { try { fn(a); } catch {} });
}

function makeTestAlert(type: string, severity: string, category: string, file: string, pattern: string, opts: Partial<Alert> = {}): Alert {
    _counter++;
    return { id: `test-${Date.now()}-${_counter}`, type, severity, category, file, pattern, timestamp: new Date().toISOString(), ...opts };
}

function getAlerts(): Alert[] { return [..._store]; }
function clearStore(): void   { _store = []; }
function onAlert(fn: (a: Alert) => void): () => void {
    _listeners.push(fn);
    return () => { const i = _listeners.indexOf(fn); if (i !== -1) { _listeners.splice(i, 1); } };
}

/* ---------- POLICY ENGINE (inline mirror) ---------------------------------- */

type PolicyAction = 'block' | 'warn' | 'log';

interface RuleMap { [key: string]: { enabled: boolean; action: PolicyAction; severity: string } }

const DEFAULT_RULES: RuleMap = {
...
function evaluatePattern(patternName: string): { action: PolicyAction; severity: string; message: string } | null {
...
function evaluateDataset(piiDensity: number, riskScore: number): { action: PolicyAction; message: string } | null {
    if (piiDensity >= 0.70) { return { action: 'block', message: `❌ PII density ${(piiDensity * 100).toFixed(0)}% exceeds block threshold` }; }
    if (piiDensity >= 0.40 || riskScore >= 60) { return { action: 'warn', message: `⚠ Dataset risk score ${riskScore.toFixed(0)}/100 — policy warning` }; }
    return null;
}

function evaluatePrompt(hasCritical: boolean, hasHigh: boolean, count: number): { action: PolicyAction; message: string } | null {
    if (hasCritical) { return { action: 'block', message: '❌ Secrets in LLM prompt — blocked by policy' }; }
    if (hasHigh || count > 0) { return { action: 'warn', message: `⚠ ${count} sensitive item(s) in prompt — policy warning` }; }
    return null;
}

/* ---------- PROMPT SCANNER (inline mirror) --------------------------------- */
...
const PROMPT_PATTERNS = [
    { name: 'Email',    regex: /\b[A-Za-z0-9._%+\-]+@[A-Za-z0-9.\-]+\.[A-Za-z]{2,}\b/g, type: 'pii',    category: 'email',    severity: 'high'     as const },
    { name: 'SSN',      regex: /\b\d{3}[-\s]\d{2}[-\s]\d{4}\b/g,                        type: 'pii',    category: 'ssn',      severity: 'critical' as const },
    { name: 'Phone',    regex: /\b(?:\+?1[-.\ ]?)?\(?\d{3}\)?[-.\ ]?\d{3}[-.\ ]?\d{4}\b/g, type: 'pii', category: 'phone',    severity: 'high'     as const },
    { name: 'API Key',  regex: /\bsk-[A-Za-z0-9]{20,}\b/g,                               type: 'secret', category: 'api_key',  severity: 'critical' as const },
    { name: 'AWS Key',  regex: /\bAKIA[0-9A-Z]{16}\b/g,                                  type: 'secret', category: 'aws_key',  severity: 'critical' as const },
    { name: 'CC',       regex: /\b(?:4\d{3}|5[1-5]\d{2})[- ]?\d{4}[- ]?\d{4}[- ]?\d{4}\b/g, type: 'pii', category: 'credit_card', severity: 'critical' as const },
    { name: 'Medical',  regex: /\b(?:diagnosed?\s+with)\s+[A-Za-z\s]{3,30}/gi,           type: 'medical', category: 'diagnosis', severity: 'critical' as const },
];

function scanPromptText(prompt: string): { findings: PromptFinding[]; isClean: boolean; hasCritical: boolean; hasHigh: boolean } {
...
function analyseDatasetHeaders(csv: string): { piiDensity: number; riskScore: number; matchedCount: number } {
...
function computeSeverity(category: string, patternName: string): string {
    if (patternName.includes('Private Key') || patternName === 'AWS Access Key' ||
        patternName === 'SSN Pattern' || patternName === 'Credit Card') { return 'critical'; }
    if (patternName.includes('API Key') || patternName === 'Password Assignment' ||
        patternName === 'Connection String' || patternName === 'JWT Token' ||
        patternName === 'Bearer Token') { return 'high'; }
    if (patternName === 'Email Address') { return 'medium'; }
    return 'low';
}
...
suite('Suite 5 — Alert Store', () => {
    ...
    test('Alerts are newest-first (LIFO order)', async () => {
        ...
        ['first', 'second', 'third'].forEach(t =>
            pushAlert(makeTestAlert(t, 'low', 'dataset_risk', 'f.csv', t))
        );
        const stored = getAlerts();
    ...
    test('Alert has required fields matching output contract', async () => {
        clearStore();
        const a = makeTestAlert(
            'secret_exposure', 'high', 'secret_exposure', 'config.ts',
            'API key detected', { line: 42, snippet: 'sk-abc...', policyAction: 'warned' }
        );
        pushAlert(a);
        const [stored] = getAlerts();
...
suite('Suite 6 — Dataset File Monitoring', () => {
    ...
    test('Policy blocks dataset with very high PII density', async () => {
        const { piiDensity, riskScore } = analyseDatasetHeaders(
            `email,phone,ssn,password,credit_card,dob,address,medical,passport,national_id,diagnosis,prescription\nalice@e.com,555-1111,123-45-6789,pass123,4111111111111111,1990-01-01,123 Main,cancer,P12345,N123456,diabetes,metformin`
        );
...
suite('Suite 7 — Alert Severity Scoring', () => {
    ...
    const SEVERITY_CASES: Array<[string, string, string]> = [
...
suite('Suite 8 — End-to-End Alert Pipeline', () => {
    ...
    test('Full pipeline: code with API key → alert → policy decision', async () => {
        ...
        for (const r of scanResults) {
            ...
            pushAlert(makeTestAlert(
                r.patternName,
                decision?.severity ?? r.alertSeverity,
                r.category === 'secret' ? 'secret_exposure' : 'pii_detected',
                'config.ts',
                r.patternName,
                { line: r.line, snippet: r.match.substring(0, 80), policyAction: decision?.action as any }
            ));
        }
    ...
    test('Full pipeline: private key → CRITICAL alert → BLOCK policy', async () => {
        ...
        for (const r of scanResults) {
            const decision = evaluatePattern(r.patternName);
            pushAlert(makeTestAlert(r.patternName, decision?.severity ?? r.alertSeverity, 'secret_exposure', 'keys.pem', r.patternName, { policyAction: decision?.action as any }));
        }
        const alerts = getAlerts();
    ...
    test('Full pipeline: prompt with email → leakage alert → WARN policy', async () => {
        ...
        const decision = evaluatePrompt(result.hasCritical, result.hasHigh, result.findings.length);
        if (decision) {
            pushAlert(makeTestAlert('Prompt leakage detected', result.hasCritical ? 'critical' : 'high', 'prompt_leakage', '<prompt>', `${result.findings.length} sensitive item(s)`, { policyAction: decision.action as any }));
        }
        const alerts = getAlerts();
    ...
    test('Alert JSON output matches Phase 4 contract schema', async () => {
        ...
        const alert = makeTestAlert(
            'secret_exposure', 'high', 'secret_exposure', 'config.ts',
            'API key detected', { line: 42, snippet: 'sk-test...', policyAction: 'warned' }
        );
        pushAlert(alert);
...
// Also export for VS Code test harness
export { runAllTests };
