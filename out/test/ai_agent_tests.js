...
function buildDatasetContext(ctx) {
    ...
    const sensitiveColumns = (l.sensitive_column_ranking ?? []).slice(0, 12).map((item) => ({
    ...
    for (const [col, drift] of Object.entries(colDrift).sort(([, a], [, b]) => b - a).slice(0, 8)) {
        if (drift > 0.15) {
            anomalies.push({ column: col, issue: 'Distribution drift', severity: drift > 0.30 ? 'high' : 'medium', detail: `JS-divergence=${drift.toFixed(4)} — synthetic distribution diverges significantly from original` });
        }
    }
    for (const cs of columnStats) {
        ...
        if (cs.type === 'numeric' && cs.mean != null && cs.std != null && Math.abs(cs.mean) > 0) {
            ...
            if (cv > 3.0) {
                anomalies.push({ column: cs.name, issue: 'High variance / skewed distribution', severity: 'medium', detail: `CV=${cv.toFixed(2)} (std/mean)` });
            }
        }
    }
    ...
    for (const cs of columnStats) {
        ...
        if (nr > 0.60) {
            cleaningSuggestions.push({ column: cs.name, issue: `${(nr * 100).toFixed(0)}% missing`, action: 'Consider dropping this column — missing rate is too high', priority: 'high' });
        }
        else if (nr > 0.30) {
    ...
    for (const an of anomalies) {
        if (an.issue === 'High variance / skewed distribution') {
            cleaningSuggestions.push({ column: an.column, issue: 'Extreme skew / outliers', action: 'Apply log1p transform or IQR-based clipping', priority: 'medium' });
        }
        if (an.issue === 'Outlier exposure risk') {
            cleaningSuggestions.push({ column: an.column, issue: 'Individual outlier exposure', action: 'Clip to 99th percentile or add Laplace noise', priority: 'high' });
        }
    }
    for (const col of piiCols) {
        const scEntry = sensitiveColumns.find(s => s.column === col);
        if (scEntry && scEntry.reidentification_risk > 0.6) {
            cleaningSuggestions.push({ column: col, issue: `Re-ID risk ${(scEntry.reidentification_risk * 100).toFixed(0)}%`, action: 'Apply k-anonymity or hashed surrogate', priority: 'high' });
        }
        else {
            cleaningSuggestions.push({ column: col, issue: 'PII detected', action: 'Mask with format-preserving pseudonymisation or remove', priority: 'medium' });
        }
    }
    // Governance actions
    const govActions = [];
    for (const sc2 of sensitiveColumns.slice(0, 6)) {
        if (sc2.pii_score > 0.7) {
            govActions.push({ column: sc2.column, action: 'Mask or tokenise', reason: `PII score ${(sc2.pii_score * 100).toFixed(0)}%`, urgency: 'high' });
        }
        if (sc2.reidentification_risk > 0.7) {
            govActions.push({ column: sc2.column, action: 'Apply k-anonymity or suppress', reason: `Re-ID risk ${(sc2.reidentification_risk * 100).toFixed(0)}%`, urgency: 'critical' });
        }
    }
    if (dir.score != null && dir.score >= 70) {
        govActions.push({ column: 'DATASET', action: 'Mandatory privacy impact assessment', reason: `Intelligence risk ${dir.score.toFixed(0)}/100`, urgency: 'critical' });
    }
    const govActionsUnique = govActions.filter((a, i, arr) => i === arr.findIndex(b2 => b2.column === a.column && b2.action === a.action));
    const threats = (l.threat_details ?? l.top_threats ?? []).map((t) => ({ name: t.name, severity: t.severity, confidence: t.confidence ?? 0, description: t.description ?? '', triggered_by: t.triggered_by ?? [] }));
    const topCorr = (g.top_correlations ?? []).slice(0, 8).map((c) => ({ cols: c.cols, pearson: c.pearson, strength: c.strength }));
    return {
...
const AgentTools = {
    get_dataset_summary(ctx) { return ctx.dataset_summary; },
    get_sensitive_columns(ctx) { return ctx.sensitive_columns; },
    get_privacy_metrics(ctx) { return { risk_metrics: ctx.risk_metrics, privacy_components: ctx.privacy_components, threats: ctx.threats }; },
    get_pii_findings(ctx) { return { pii_columns: ctx.pii_columns, cleaning_suggestions: ctx.cleaning_suggestions.filter(s => ctx.pii_columns.includes(s.column)), governance_actions: ctx.governance_actions }; },
    get_recent_alerts(ctx) { return ctx.recent_alerts; },
    get_anomalies(ctx) { return ctx.anomalies; },
    get_column_stats(ctx, col) { return col ? ctx.column_stats.filter(c => c.name === col) : ctx.column_stats; },
    get_sql_schema(ctx) {
        return {
...
function suite(label) { process.stdout.write(`\n▶ ${label}\n`); }
function eq(a, b, msg) { if (a !== b) {
    throw new Error(msg ?? `Expected ${JSON.stringify(b)} but got ${JSON.stringify(a)}`);
} }
function ok(cond, msg) { if (!cond) {
    throw new Error(msg ?? 'Assertion failed');
} }
function gt(a, b, msg) { if (!(a > b)) {
    throw new Error(msg ?? `Expected ${a} > ${b}`);
} }
function gte(a, b, msg) { if (!(a >= b)) {
    throw new Error(msg ?? `Expected ${a} >= ${b}`);
} }
function contains(s, sub, msg) { if (!s.includes(sub)) {
    throw new Error(msg ?? `Expected "${s.substring(0, 80)}" to contain "${sub}"`);
} }
function notNull(v, msg) { if (v == null) {
...
//# sourceMappingURL=ai_agent_tests.js.map